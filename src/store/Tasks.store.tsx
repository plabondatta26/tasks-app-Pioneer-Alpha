...
const fetchData = async (): Promise<Task[]> => {
  try {
    const response = await fetch("http://localhost:8000/api/all/task/");
...
async function fetchDirectories(): Promise<string[]> {
  try {
    const response = await fetch("http://localhost:8000/api/all/deractory/");
    const data = await response.json();

    data.forEach((element: any) => {
      if (!dirList.includes(element.title)) {
        dirList = [...dirList, element.title];
      }
    });
...
async function fetchDataAndDirectories() {
  try {
